...
<body>
	...
	if (u == 2) {
		query1 = "SELECT sid from seller WHERE sid=?";
		query2 = "INSERT INTO seller(sid,pass,fname,lname,email,address,phno) VALUES(?,?,?,?,?,?,?)";
	} else if (u == 1) {
		query1 = "SELECT uid from customer WHERE uid=?";
		query2 = "INSERT INTO customer(uid,pass,fname,lname,email,address,phno) VALUES(?,?,?,?,?,?,?)";
	} else if (u == 0) {
		query1 = "SELECT aid from admin WHERE aid=?";
		query2 = "INSERT INTO admin(aid,pass,fname,lname,email,address,phno) VALUES(?,?,?,?,?,?,?)";
	}
